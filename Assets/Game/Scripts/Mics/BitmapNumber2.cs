...
public class BitmapNumber2 : MonoBehaviour
{
    ...
    void Update()
    {
        var color = new Color(_signImage.color.r, _signImage.color.g, _signImage.color.b, _alpha);
    ...
    {
        float width = _signImage.bounds.size.x;
        for(int i = 0; i < _numberImages.Count; i++)
        {
            width += _numberImages[i].bounds.size.x;
        }
        return width;
    }
}
